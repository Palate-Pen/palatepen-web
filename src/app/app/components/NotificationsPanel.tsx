...
// Pure panel content — header row + list. Used by NotificationsTab (sidebar
// popout) and the mobile NotificationsSheet. No internal positioning so the
// caller controls layout entirely.
export function useNotificationsModel() {
  ...
  const notifications = useMemo<Notif[]>(() => {
    ...
    return out.filter(n => !dismissed.has(n.id)).sort((a, b) => b.ts - a.ts).slice(0, 50);
  }, [state, sym, dismissed]);

  const unreadCount = notifications.filter(n => n.ts > lastSeen).length;
  ...
  function clearAll() {
    ...
    const merged = Array.from(new Set([...dismissedIds, ...ids])).slice(-500);
    actions.updProfile({ notificationsDismissedIds: merged });
  }
  function dismissOne(id: string) {
    const merged = Array.from(new Set([...dismissedIds, id])).slice(-500);
    actions.updProfile({ notificationsDismissedIds: merged });
  }
  function markRead(ts: number) {
    if (ts > lastSeen) actions.updProfile({ notificationsLastSeen: ts });
  }

  return { notifications, lastSeen, unreadCount, markAllRead, clearAll, dismissOne, markRead };
}

export default function NotificationsPanel({ onJump }: { onJump: (goTo: string) => void }) {
  const { settings } = useSettings();
  const C = settings.resolved === 'light' ? light : dark;
  const { notifications, lastSeen, unreadCount, markAllRead, clearAll, dismissOne, markRead } = useNotificationsModel();
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: 0, flex: 1 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', borderBottom: '1px solid ' + C.border, gap: '8px' }}>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          {unreadCount > 0 && (
            <button onClick={markAllRead} style={{ fontSize: '11px', color: C.gold, background: 'transparent', border: 'none', cursor: 'pointer', padding: 0 }}>Mark all read</button>
          )}
          {notifications.length > 0 && (
            <button onClick={clearAll} style={{ fontSize: '11px', color: C.faint, background: 'transparent', border: 'none', cursor: 'pointer', padding: 0 }}>Clear all</button>
          )}
        </div>
      </div>
        ...
        {notifications.length === 0 ? (
          ...
          notifications.map(n => {
            ...
            return (
              ...
              >
                <button onClick={() => jump(n)} style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', flex: 1, textAlign: 'left', background: 'transparent', border: 'none', cursor: 'pointer', padding: 0, color: C.text }}>
                  <span style={{ width: '24px', height: '24px', borderRadius: '4px', background: meta.color(C) + '20', border: '0.5px solid ' + meta.color(C) + '40', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '13px', color: meta.color(C), flexShrink: 0, fontFamily: 'Georgia,serif', fontWeight: 700 }}>{meta.icon}</span>
                ...
                <button onClick={() => dismissOne(n.id)} title="Dismiss" style={{ background: 'transparent', border: 'none', color: C.faint, fontSize: '14px', cursor: 'pointer', padding: '0 2px', flexShrink: 0, marginTop: '2px' }}>×</button>
              </div>
            );
          })
        )}
