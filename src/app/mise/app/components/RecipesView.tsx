...
export default function RecipesView() {
  ...
  const linkedNotes = sel
    ? (sel.linkedNoteIds||[]).map((id:string) => state.notes.find((n:any) => n.id === id)).filter(Boolean)
    : [];
  ...
  async function importUrl() {
    ...
    setImporting(true);
    try {
      const res = await fetch('/api/mise/import-recipe', {
        method: 'POST',
  ...
  function openStock() {
    ...
    setStockIngs(sel.imported.ingredients.map((ing:string, i:number) => ({
      id: String(i), name: ing,
      exists: existing.includes(ing.toLowerCase()),
      selected: !existing.includes(ing.toLowerCase()),
    })));
    setShowStockModal(true);
  }

  function addToStock() {
    stockIngs.filter(i => i.selected).forEach(ing =>
      actions.addStock({ name:ing.name, unit:'kg', parLevel:null, minLevel:null, currentQty:null })
    );
    ...
    <div className="flex h-screen font-epilogue">
      <div className="flex-1 overflow-auto p-8">
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {[sel.category, sel.imported?.servings ? 'Serves '+sel.imported.servings : null,
            sel.imported?.prepTime ? 'Prep: '+sel.imported.prepTime : null].filter(Boolean).map((t:string) => (
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-mise-surface border border-mise-border w-full max-w-md max-h-[80vh] flex flex-col">
            <div className="flex justify-between items-start p-6 border-b border-mise-border">
              ...
              <button onClick={() => setShowStockModal(false)} className="text-mise-faint hover:text-mise-dim text-xl">&times;</button>
            </div>
            <div className="overflow-auto flex-1 p-4 space-y-2">
              {stockIngs.map(ing => (
                <button key={ing.id}
                  onClick={() => setStockIngs(prev => prev.map(i => i.id === ing.id ? {...i, selected:!i.selected} : i))}
          ...
          {filtered.map((r:any) => (
            <button key={r.id} onClick={() => setSel(r)} className="text-left bg-mise-surface border border-mise-border p-5 hover:border-mise-gold hover:bg-mise-surface2 transition-colors group">
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                {r.imported && <span className="text-xs font-bold tracking-widests uppercase text-mise-faint bg-mise-surface3 border border-mise-border px-2 py-0.5">Imported</span>}
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-mise-surface border border-mise-border w-full max-w-lg">
            <div className="flex justify-between items-center p-6 border-b border-mise-border">
              ...
              <button onClick={() => setShowAdd(false)} className="text-mise-faint hover:text-mise-dim text-xl">&times;</button>
            </div>
            <div className="p-6 space-y-4">
              {tier === 'pro' ? (
                <div>
                  ...
                  {imported && <p className="text-xs text-mise-green mt-2">&#10003; Recipe imported &mdash; {imported.ingredients?.length||0} ingredients</p>}
                </div>
              ) : (
                <div className="bg-mise-gold/10 border border-mise-gold/20 p-3">
                  <p className="text-xs text-mise-gold">&#x1F512; URL import is a Pro feature &mdash; upgrade to import recipes automatically</p>
                </div>
              )}
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {CATS.map(c => (
                    <button key={c} onClick={() => setNewCat(c)}
                      className={`text-xs px-3 py-1.5 border transition-colors ${newCat === c ? 'border-mise-gold text-mise-gold bg-mise-gold/10' : 'border-mise-border text-mise-dim hover:border-mise-border-light'}`}>
                      {c}
                    </button>
                  ))}
                </div>
