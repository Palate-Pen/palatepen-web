...
import { draftTransferAction } from '../actions';

export const metadata = { title: 'New Transfer — Palatable' };
...
export default async function NewTransferPage() {
  ...
  // Current site first, others after.
  sites.sort((a, b) => {
  ...
  return (
    <div className="px-4 sm:px-8 lg:px-10 pt-6 lg:pt-12 pb-12 lg:pb-20 max-w-[960px] mx-auto">
      ...
      <h1 className="font-serif text-4xl font-normal leading-[1.1] tracking-[-0.015em] text-ink mb-3 mt-4">
      ...
      <form action={draftTransferAction} className="bg-card border border-rule px-7 py-7">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          <div>
            ...
            <div className="font-serif text-sm text-ink mb-2">
              {ctx.kitchenName}
            </div>
            <select
              name="source_pool"
        ...
        <textarea
          name="notes"
          rows={3}
          placeholder="e.g. bar running low on olive oil for Friday service"
        ...
        <div className="flex items-center justify-between pt-4 border-t border-rule">
          <p className="font-serif italic text-sm text-muted">
            Same site? Kitchen ↔ bar within {ctx.kitchenName} is fine — inventory is shared so it's recorded but no stock figure changes.
          </p>
          <button
            type="submit"
