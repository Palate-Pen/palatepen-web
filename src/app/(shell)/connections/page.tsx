...
function ServiceCard({ service }: { service: Service }) {
  return (
    <div className="bg-card border border-rule px-7 py-7 text-center cursor-pointer transition-colors hover:border-gold">
      ...
      <div className="text-xs text-muted mb-4">{service.status}</div>
      <button
        className={
          service.connected
            ? 'font-sans font-semibold text-xs tracking-[0.08em] uppercase px-5 py-2.5 bg-transparent text-gold border border-gold'
            : 'font-sans font-semibold text-xs tracking-[0.08em] uppercase px-5 py-2.5 bg-gold text-white border-0'
        }
      >
