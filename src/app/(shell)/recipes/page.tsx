...
export default function RecipesPage() {
  return (
    <div className="px-14 pt-12 pb-20 max-w-[1400px]">
      <h1 className="font-serif text-5xl text-ink mb-3">Recipes</h1>
      <p className="font-serif italic text-[17px] text-muted mb-8">
  ...
  return (
    <div className="bg-card border border-rule cursor-pointer transition-all hover:border-gold hover:shadow-[0_4px_16px_rgba(26,22,18,0.08)] flex flex-col">
      <div className="px-6 py-6 border-b border-rule">
        ...
        <div className="text-[11px] text-muted mt-2 tracking-[0.02em]">
      ...
      <div className="px-6 py-4 border-b border-rule flex justify-between items-center bg-gradient-to-r from-[rgba(93,127,79,0.06)] to-transparent">
        ...
        <div className="text-right">
          ...
          <div className="text-[10px] text-muted mt-0.5">{recipe.totalCost}</div>
        </div>
      </div>
      ...
      <div className="px-6 py-5 flex-1">
        {recipe.ingredients.map((ing, i) => (
          ...
          >
            <div className="font-serif text-[13px] text-ink flex-1">{ing.name}</div>
            <div className="text-[10px] text-muted w-20 text-right">{ing.qty}</div>
...
function RecipeButton({ children }: { children: React.ReactNode }) {
  return (
    <button className="flex-1 py-2 font-display font-semibold text-[8px] tracking-[0.3em] uppercase bg-transparent border border-rule text-muted transition-colors hover:border-gold hover:text-gold">
      {children}
    </button>
  );
}
