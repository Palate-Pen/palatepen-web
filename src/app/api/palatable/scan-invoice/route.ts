...
export async function POST(req: NextRequest) {
  try {
    ...
    // Admin/operator emails are always 'enterprise' for gating purposes —
    // see src/lib/adminEmails.ts.
    const tier = isAdminEmail(user.email) ? 'enterprise' : (user.user_metadata?.tier || 'free');

    const denial = await denyIfBlocked(tier, 'invoices_ai_scan', 'aiInvoiceScan');
