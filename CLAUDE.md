...
- Tiers: Free / Pro £49pm / Kitchen £79pm / Group £119pm / Enterprise (price on request — contact_sales route to hello@palateandpen.co.uk). Per site, per month. Add the Safety module to any tier for £20/site/mo more.
...
- [x] Seed founder demo with Safety data — *(superseded 2026-05-18 by per-account populate. `/admin/accounts/[id]` now exposes a Populate demo data button gated to is_founder/is_demo. Calls `v2.populate_demo_account` RPC which wipes + reseeds 30 days across every surface incl. full safety stack: 29 of 30 opening checks · 30 probe readings (2 fail) · 14 cleaning tasks + 33 signoffs · 4 training (mixed expiry) · 2 incidents. Idempotent. Migration: `20260518_v2_populate_demo_account_rpc.sql`.)*
...
**Last shipped (2026-05-18 session — per-account populate button):**
- `/admin/accounts/[id]` now exposes a Populate demo data button (gated to is_founder OR is_demo accounts; renders nothing on customer accounts — defence in depth on the action gate). Two-click confirm: arms gold, then `Confirm — wipe & reseed` in urgent red. On success, per-site count strip across every domain.
- New `v2.populate_demo_account(p_account_id)` RPC. Loops every site of the account, wipes per-site v2.* tables in FK-safe order, then dispatches to one of two helpers by `sites.kind`:
  - `_seed_demo_kitchen_site` — 6 suppliers · 18 ingredients (4 below par) · 72 price-history rows · 12 recipes (1 stale @35d for stale_cost_baseline) · 22 recipe_ingredients · 1 menu plan + 8 items · 6 invoices + 24 lines (1 flagged) · 2 POs + 6 lines · 1 credit note · 1 stock take + 6 lines · 12 prep · 10 waste · 5 notebook · 29 of 30 opening checks · 30 probe readings (2 fail on sauce-station fridge) · 14 cleaning tasks + 33 signoffs · 4 training (mixed expiry bands) · 2 incidents
  - `_seed_demo_bar_site` — 5 suppliers · 15 ingredients · 60 price-history · 10 drinks (6 cocktails + 2 wine + 1 beer + 1 soft, 1 stale @32d) · 18 recipe_ingredients · 1 plan + 6 items · 5 invoices + 15 lines · 1 PO + 3 lines · 6 prep · 8 spillage · 3 notebook
- Detectors run per site after the populate so Looking Ahead bars are live on first paint.
- Migration `20260518_v2_populate_demo_account_rpc.sql` (940 lines, applied via MCP in 3 parts due to 50KB+ payload size). Three post-apply patches stamped in the header: menu_plans.surface enum (`chef`/`bartender` → `kitchen`/`bar`), menu_plans.status (`in_progress` → `draft`), safety_incidents.allergens (`text[]` → `v2.allergen_code[]`).
...
- **Customer demo account: hello@palateandpen.co.uk.** Public-facing live demo for customer walkthroughs. State: `accounts.is_demo = true`, `tier = enterprise`, sites Demo Kitchen + Demo Bar under account `42127327-20a2-47a2-a255-8a0ebf4bb772`. Initial 30-day seed (2026-05-17) populates every surface — Bank, Recipes, Prep, Invoices, Waste, Notebook, Safety. Stripe must treat `is_demo` the same as `is_founder` — never charge. Memory entry: `palatable-hello-demo-account.md`. Migration: `20260517_v2_accounts_is_demo.sql`. Daily re-anchoring cron + admin reseed button still pending (Slice B).
