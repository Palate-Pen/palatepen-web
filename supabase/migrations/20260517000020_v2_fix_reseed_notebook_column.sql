-- v2 migration: fix v2.reseed_founder_demo() column reference
-- Date: 2026-05-17
-- Applied: 2026-05-17 (npx supabase db push --include-all)
--
-- Bug: 20260515000014 referenced v2.notebook_entries.captured_at when
-- shifting timestamps on /admin/ops reseed. That column doesn't
-- exist — the table's timestamp column is created_at (defined in
-- 20260514000012_v2_notebook_entries.sql).
--
-- Symptom: clicking Reseed in /admin/ops returns
--   rpc_failed: column "captured_at" does not exist
-- The same delegation chain runs from reseed_demo_account so hello@
-- and any future is_demo account also failed.
--
-- Fix: CREATE OR REPLACE the function with created_at on the notebook
-- update. Every other table reference in the function is correct and
-- left as-is.
...
begin
  ...
  -- FIXED: was captured_at, the actual column is created_at.
